...
byte brightness = 25;  // начальная частота ШИМ (RX_LED-255) = 10%
...
void setup() {   ///////////// Н А С Т Р О Й К А (сетап) ////////////////////////////////
  ...
  pinMode(2, INPUT_PULLUP);   // пин 2 вход данных частотомер\4 для V8

#define KEY_1  PB0 // 8-A0- V8 12
#define KEY_2  PB2 // 10-A1- V8 10
#define KEY_3  PB4 // 12-12- V8 8
#define BEEP   PC2 // A2-A2- V8 A2
#define GEN0   PC0 // A0-10- V8 9
#define GEN1   PB1 // 9-9- V8 13
#define RX_LED PD2 // 2- V8 0
#define TX_LED PD3 //  3- V8 1
#define LED    PB5 //  13- V8 13
 
  pinMode(KEY_3, INPUT_PULLUP);  // 3 кнопка
  pinMode(KEY_2, INPUT_PULLUP);  // 2 кнопка
  pinMode(KEY_1, INPUT_PULLUP);  // 1 кнопка
  
  pinMode(BEEP, OUTPUT);        // пин а0 пищалка+++
  pinMode(GEN0, OUTPUT);        // пин 10 выход на генератор -
  pinMode(GEN1, OUTPUT);         // пин 9 выход на генератор + +++++++
  
  pinMode(RX_LED, OUTPUT);         // светодиод на плате тх
  pinMode(TX_LED, OUTPUT);         // светодиод а плате рх
  pinMode(LED, OUTPUT);        // пин 13 светодиод а плате
  digitalWrite(TX_LED, HIGH);      // выключаем изначально светордиод рх
  ...
  tone(BEEP, 349, 204.54525);    // Проигрываем мелодию
  delay(227.2725);
  tone(BEEP, 349, 204.54525);
  delay(227.2725);
  tone(BEEP, 349, 102.272625);
  delay(113.63625);
  tone(BEEP, 391, 102.272625);
  delay(113.63625);
  tone(BEEP, 329, 204.54525);
  delay(227.2725);
  tone(BEEP, 329, 204.54525);
  delay(227.2725);
  tone(BEEP, 293, 102.272625);
  delay(113.63625);
  tone(BEEP, 261, 102.272625);
  delay(113.63625);
  tone(BEEP, 293, 306.817875);  // конец мелодии
}
void loop() {  ////// Г Л А В Н Ы Й  Ц И К Л  П Р О Г Р А М М Ы ///////////////////////////////////
  ///////////////////////// К Н О П К И //////////////////////////
  ///////////////// главное меню 1 кнопка перемещение вправо /////////////////////////////
  if (digitalRead(KEY_1) == 0) { // 1 верхняя кнопка, обработка нажатия
    u8g2.setFont(u8g2_font_10x20_mr);
    pwmWrite(led, 0);
    SetPinFrequency(led, 0);
    digitalWrite(GEN1, 0); // отключаем на всякий случай питание постоянное этого пина
    digitalWrite(GEN0, 0); // отключаем на всякий случай питание постоянное этого пина
    digitalWrite(TX_LED, HIGH); // Гасим светодиоды тх rx
    digitalWrite(RX_LED, HIGH); // Гасим светодиоды тх rx
    ...
    if (digitalRead(KEY_1) == 1) {  // отпускание кнопки в главном меню
  ...
  if (r == 0 && digitalRead(KEY_3) == 0) {
    u = 0;
    b++;
    delay(300) ;
    if (digitalRead(KEY_3) == 1) {
      tone(s, 500, 100);
      q--;
      b = 0;
    }
  ...
  if (r == 0 && q == 1 && digitalRead(KEY_2) == 0) {
  ...
  if (r == 0 && q == 2 && digitalRead(KEY_2) == 0) {
    b++;  // счетчик для понимания удерживается ли кнопка
    if (y == 0 && b >= 4) {
      u8g2.firstPage();
      u8g2.setCursor(0, 13);
      y = 35;
      tone(GEN1, y);  // на 9 пине создаем сигнал 35 герц
    ...
    if (y == 35 && b >= 4) {
      ...
      u8g2.setCursor(0, 13);
      y = 700;
      tone(GEN1, y);  // на 9 пине создаем сигнал 700 герц
  ...
  if (r == 0 && q == 3 && digitalRead(KEY_2) == 0) {
    b++;  // счетчик для понимания удерживается ли кнопка
    if (s == 16 && b >= 4) {
      u8g2.firstPage();
      u8g2.setCursor(0, 13);
  ...
  if (r == 0 && q == 4 && digitalRead(KEY_2) == 0) {
    b++;  // счетчик для понимания удерживается ли кнопка
    if (x == 0 && b >= 4) {
      u8g2.firstPage();
      u8g2.setCursor(0, 13);
  ...
  if (r == 0 && q == 5 && digitalRead(KEY_2) == 0) {
    b++;  // счетчик для понимания удерживается ли кнопка
    if (f == 0 && b >= 4) {
      tone(s, 700, 100);
      f = 1;
  ...
  if (r == 0 && q == 7 && digitalRead(KEY_2) == 0) {
    b++;  // счетчик для понимания удерживается ли кнопка
    if (j == 0 && b >= 4) {
      tone(s, 700, 100);
      j = 1;
  ...
  if (j == 1 && millis() - timer > 30000) { //  """""""4 пин выключаем после 30 секунд бездействия фонарь"""""
    digitalWrite(4, 0);
    j = 0;
  }  
  ...
  if (e == 2 && digitalRead(KEY_2) == 0 ) {
    digitalWrite(GEN0, 0); // выключаем подачу минуса на иглу на всякий случай
    tone(s, 700, 100);
    if (digitalRead(GEN1) == 0) digitalWrite(GEN1, 1);  // включаем подачу плюса на иглу
    else digitalWrite(GEN1, 0);  // выключаем подацу плюса на иглу
    delay(500);
  }
  ////////////////  режим вольтметр 3 кнопка подключение нагрузки  //////////////////////////
  if (e == 2 && digitalRead(KEY_3) == 0 ) {
    digitalWrite(GEN1, 0); // выключаем подачу плюса на иглу на всякий случай
    tone(s, 700, 100);
    if (digitalRead(GEN0) == 0) digitalWrite(GEN0, 1); //включаем подачу минуса на иглу
    else digitalWrite(GEN0, 0);  // выклюаем подачу минуса на иглу
    delay(500);
  }
  ////////////// режиме осциллограф 2 кнопка вольтаж  /////////////////////////////
  if (e == 3 && digitalRead(KEY_2) == 0) {
    b++;
    delay(500);
    if (digitalRead(KEY_2) == 1) {
      tone(s, 500, 100);
    ...
    if (b >= 3 && digitalRead(GEN1) == 0) {  //   УДЕРЖАНИЕ КНОПКИ
      digitalWrite(GEN1, 1) ;
      tone(s, 2000, 300);
      delay(500);
      b = 0;
    }
    if (b >= 3 && digitalRead(GEN1) == 1) {
      digitalWrite(GEN1, 0) ;
      tone(s, 500, 300);
      delay(500);
      b = 0;
  ...
  if (e == 3 && digitalRead(KEY_3) == 0 ) {  // если мы в осциллографе и нажата кнопка 3
  ...
  if (e == 5 && digitalRead(KEY_2) == 0) {
    tone(s, 700, 100);
    b++;
    if (b >= 3) k = k + 5;     // прибавляем по 5 к таймеру при удержании кнопки
    else k++;
    if (k >= 100)k = 1;
    brightness = (255 / 100) * k;  // переводим проценты в в реальный ШИМ (RX_LED-255)
    delay(500);
    if (digitalRead(KEY_2) == 1)b = 0;  // обнуляем счетчик удержания кнопки
  }
  //////////////  режим генератор 3 кнопка частота /////////////////////////////
  if (e == 5 && digitalRead(KEY_3) == 0) {
    tone(s, 700, 100);
    b++;
  ...
  if (e == 7 && digitalRead(KEY_2) == 0) {
    ...
    delay(500);
    if (digitalRead(KEY_2) == 1)b = 0;
  }
  ////////////////////  режим таймер 3 кнопка, время подачи //////////////////////////
  if (e == 7 && digitalRead(KEY_3) == 0) {
    tone(s, 700, 100);
    b++;
  ...
  if (e == 9 && digitalRead(KEY_2) == 0) {
    tone(s, 2000, 500);
    EEPROM.update(RX_LED, t);  // сохраняем значение в епром память ардуино
    delay(500);
  }
  //////////////////////// Режим kor 3 кнопка  (внутри функции коррекция) //////////////////
  if (e == 9 && digitalRead(KEY_3) == 0) {
    tone(s, 700, 100);
  ...
  if (millis() - timer > 3000 && q == 7 && r == 0)  { // если выбран дпкв
    ...
    noTone(GEN1); // выключаем проверочный генератор
    dpkv();
  }
  //////////////// correction
...
void voltmetr() { // функция вольтметр
  ...
  u8g2.setCursor(0, 13);
  u8g2.print("V");
  if (digitalRead(GEN0) == 1) { // если на 10 пине высокий уровень то пишем N (нагрузка)
    u8g2.setCursor(0, 32);
    u8g2.print("N");
  }
  digitalWrite (LED, 0);//???????? 
  if (digitalRead(GEN1) == 1) { // если на 9 пине высокий уровень то пишем +
    u8g2.setCursor(0, 32);
    u8g2.print("+");
    if (Htime <= 5) { // если напряжение просело то пищим
      tone(s, 700, 150);  // подключаем пищалку на прозвонке
      digitalWrite (LED, 1);  // пождключаем светодиод на плате ардуино на прозвонке
    }
  }
  if (Htime >= 200 && Htime <= 250) digitalWrite(RX_LED, LOW); // 2 строки светим светодиодом rx  если на игле около 12 вольт
  else digitalWrite(RX_LED, HIGH);
  if (Htime >= 70 && Htime <= 100) digitalWrite(TX_LED, LOW); // 2 строки светим светодиодом  tx если на игле около 5 вольт
  else digitalWrite(TX_LED, HIGH);
